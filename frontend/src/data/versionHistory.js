export var VERSION_HISTORY = [
  {
    ...
    internalChanges: [
      ...
      'Post-prep scope addition: PR #952 fixed #942 by adding deterministic bench-history balancing to lineupEngineV2; PR #956 added eight focused test files without changing runtime behavior. Candidate and 24-hour soak were re-anchored to develop merge 1da474e on 2026-08-30.',
  ...
  },
  {
    ...
    internalChanges: [
      ...
      'Routine dependency updates: express-rate-limit, @vitest/ui, jsdom, @supabase/supabase-js (PRs #671, #672, #627, #670).',
      'Bumped CI\'s pinned Node runtime from 20 to 22 on the frontend/backend/backend-unit/sync-script/rls jobs (PR #678) — jsdom 30 and the current @supabase/supabase-js sub-packages both raised their engines floor to Node >=22, which CI running Node 20 could no longer satisfy. Verified behavior-neutral: identical pass counts (134/134 backend unit, 1056/1057 frontend) on Node 22 against unmodified develop before merging. No app code or dependency changes in that PR itself.',
      'Documentation accuracy pass: corrected three ROADMAP.md story statuses that still read "Open" after shipping (Stories 120, 124, 126 — PR #675), corrected CLAUDE.md\'s v2.9.0 note, which still described that release as "not yet promoted to main" a week after it actually promoted (PR #661, 2026-08-09), fixed a stale file path in the Phase 4C shim-removal checklist left over from the Slice 4 ScoringMode refactor, and flagged a second, independent GRANT-level gap on the live-scoring tables — confirmed via a direct read-only prod query — separate from the RLS-policy work tracked under #355 (PR #676).',
    ],
  },
  {
    version: '2.9.0',
  ...
  },
  {
    ...
    internalChanges: [
      'Completed retiring the legacy color object (var C) in App.jsx: modals/overlays, the Schedule tab, the Lineups + Links tabs, the Feedback/About/Account/Updates tabs, the public SharedView share-link page, and a final sweep of 5 render functions outside the original planned regions (renderSongs, renderSnackDuty, renderPinModal, renderTeamTab, renderBottomNav, plus 2 literal-hex sites) — all zero-visible-change token swaps (#528, #529, #537, #545, #546, #547, Story 120/#531, #606). var C now has zero remaining call sites in App.jsx.',
      ...
      'Corrected stale documentation found during an audit pass: frontend/CLAUDE.md\'s test count and its outdated tab-list description, and a DOC_TEST_DEBT.md disambiguation note (#607/#608, #610/#611).',
      'Added 18 new tests across the token-migration and extraction work (mutation-tested where the change was value-preserving, not behavior-changing).',
  ...
  },
  {
    ...
    techNote: 'Under-the-hood stability improvements',
    internalChanges: [
      'Phase 2 primitives shipped: Badge, Button, Card, Stack, Text components in frontend/src/components/ui/',
      '107 new component tests across 5 primitive .test.jsx files (Badge.test.jsx, Button.test.jsx, Card.test.jsx, Stack.test.jsx, Text.test.jsx)',
      'LockFlow.jsx: removed dead duplicate fontSize:"13px" declaration in lock-confirmation header — second 10px declaration was already winning, no visual change',
      'PlayerHandBadge.jsx migrated to use Badge primitive (Phase 3 Step 1, PR #62) — first consumer of v2.5.10 primitives, pure refactor with no visual change',
    ],
  },
  {
    version: '2.5.9',
