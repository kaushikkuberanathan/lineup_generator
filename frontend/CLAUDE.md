...
- **Framework**: Vitest
- **CI target**: 1422 frontend passed / 1 skipped / 0 failed (124 test files; backend unit 276 — see `backend/CLAUDE.md` § Test Suite for that count's own history), directly measured via `npx vitest run --no-file-parallelism` on `develop` HEAD 2026-08-29. Corrected twice the same day during a v3.0.0 release-ritual audit: first from the stale 1390/120-files figure (2026-08-27, #112/#116/#120 governance batch) to 1401/122 as of v3.0.0's own promote, then to 1422/124 after PR #907 (Terms of Service experience — separate work, unrelated to v3.0.0, landed on `develop` the same day) added its own frontend coverage on top. The prior entry's own file-by-file breakdown (Help redesign +9, Group 6 DB-cache precedence +6, Group 5 dual-write +6, banned-token scanner +1) is left as historical record below rather than re-derived. +22 vs. the 2026-08-26 baseline of 1368: +9 from the v2.15.1 Help redesign (`FAQSection.test.jsx` 6→15, already on `develop` when this batch's branch was cut, not part of this batch's own work) plus +13 from this batch itself — Group 6 `accessibility.v1.test.js` DB-cache precedence (#112, +6), Group 5 `flag-bootstrap.test.js` dual-write (#120, +6), `versionHistory.test.js` banned-token scanner (#116, +1).
