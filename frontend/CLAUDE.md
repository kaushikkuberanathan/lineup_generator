...
- **Framework**: Vitest
- **CI target**: **1852 frontend passed / 0 failed across 186 test files** (backend unit 361/361, unchanged — see `backend/CLAUDE.md` § Test Suite), directly re-measured on `release/v3.4.0` during v3.4.0 release prep — the +93/+17 growth is the Contemporary UX Redesign wave (initiative #1052: My Team/Roster, Schedule, Game Day entry/readiness, Defense, Batting, Walk-up Songs, Support/Account) plus the More-tab regroup (Story 341/#1099), all landed on `develop` between v3.3.3 and this release. Before that, 1759/169 after PR #1061 (#1060 fix, +2 assertions to `auth.test.js`), PR #1064 (#1062 fix, new `networkHealth.test.js` + 3 assertions to `api.client.test.js` + 2 more to `auth.test.js`). Prior baseline was 1748/168 after PR #1050 (#1049 fix) added one regression test to `AppApiDrivenHomeIntegration.test.jsx`. Before that, 1745/168 after PR #1035 merged the API-driven architecture redesign's Phase 0 + Phase 1 (#1012, 22 new test files across `frontend/src/api/`, `frontend/src/features/home/`, and `AppApiDrivenHomeIntegration.test.jsx`). Everything new ships behind two default-off flags (`API_DRIVEN_HOME`, `API_DRIVEN_ROUTES`) — zero behavior change for any current user. Before that, 1550/146 on the local v3.2.0 release-prep branch after PR #1001 added `AppMyTeamDashboard.test.jsx` (3 tests) and updated the Roster, Schedule, Bottom Nav, and adjacent App mocks. Before that, 1547/145 on Wave 2 after adding one `LiveScoringPanel.test.jsx` regression for the resolved stale pitch-history copy (#107) and three `analyticsIdentity.test.js` cases for deferred Mixpanel identity (#266). Before that, the baseline was 1543/144 after adding `AppRosterGoldenPath.test.jsx`, `AppPinModalGoldenPath.test.jsx`, and `AppBottomNavGoldenPath.test.jsx` (#943 — QA & Reliability Audit finding: Roster tab, PIN modal, and Bottom Nav were 3 of the 6 named untested App.jsx render paths; 10 new tests). **#943 itself stays open by design** — it's a standing structural-decomposition tracker.
...
| `AppDemoTeamGoldenPath.test.jsx` | `loadDemoTeam()` golden path (#969): fresh demo creation via a real "Try Demo Team" click (roster/schedule/grid/innings/batting counts vs. the real `demoSeed.js` constants + `demoSeedVersion`), the dedup guard at the current seed version, and an existing older-version demo team — documents that the button-visibility guard hides "Try Demo Team" by name only, making `loadDemoTeam()`'s own version-comparison/cleanup branch currently unreachable via the UI (3 tests) |
...
| `AppRosterGoldenPath.test.jsx` | Roster tab (App.jsx renderRoster) golden path (#943): existing roster renders sorted alphabetically, the Add Player form adds a new title-cased player via a real form submission, and a blank first/last name is silently rejected (form stays open, no partial entry added) (3 tests) |
| `AppPinModalGoldenPath.test.jsx` | Coach-PIN unlock golden path (#943, "unlock" mode only — the other 5 `pinModal` modes share the same render/submit path): wrong PIN shows an inline error and keeps the lineup locked, correct PIN closes the modal and unlocks the lineup, backdrop/Cancel dismisses without unlocking (3 tests) |
| `AppBottomNavGoldenPath.test.jsx` | Bottom Nav golden path (#943): ordinary tab switching across My Team/Game Day/Support, plus the non-obvious Exit Sheet gate — tapping Home while inside My Team opens a "Leave team?" confirmation instead of navigating away, with Keep Working staying put and Go to Home Screen actually leaving (4 tests) |
| `AppApiDrivenHomeIntegration.test.jsx` | API-driven Home integration (#1012 Phase 1, `API_DRIVEN_HOME`/`API_DRIVEN_ROUTES`, both default off): flag-off renders unchanged legacy Home; flag-on renders the new Team Hub; CTA tap navigates + pushes a canonical route; Back returns to Home with the same team expanded; no-membership empty state reaches TeamSearch; direct deep-link open; foreign teamId never enters any team; cross-team `gameId`/`lineupId` rejected both with and without a primed Home cache (a real gap found and fixed this session, RED-to-GREEN verified via `git stash`); pending-destination resume across an auth round trip, plus a negative control for no-false-resume (18 tests) |
| `networkHealth.test.js` | `networkHealth.js`'s self-correcting connectivity signal (#1062, v3.3.3): a single reported failure doesn't flip state, two consecutive ones do; a reported success resets the failure count and restores online; subscribers notified only on an actual state change, with the new value, and not after unsubscribing; a real browser `offline`/`online` event flips state immediately even on the first occurrence and resets the failure count (6 tests) |
...
| `COMBINED_GAMEMODE_AND_SCORING` | `true` | DugoutView — combined Game Mode + Scoring surface. GA default-on as of Slice 3 (v2.5.9). Legacy ScoringMode removed. Roll back per-user: `localStorage.setItem("flag_COMBINED_GAMEMODE_AND_SCORING", "false")` |
| `API_DRIVEN_HOME` | `false` | #1012 Phase 1 — renders the API-driven Team Hub (`features/home/HomeScreen.jsx`, sourced from `GET /api/v1/home`) instead of the legacy Home screen. Try per-user: `localStorage.setItem("flag_API_DRIVEN_HOME", "true")`. Rollout: Stage 3 (internal cohort) performed 2026-09-03 against real prod — see `docs/product/ROADMAP.md` § API-Driven Architecture Redesign and issue #1033. Stage 4 (limited authenticated cohort) not yet started |
| `API_DRIVEN_ROUTES` | `false` | #1012 Phase 1 — activates canonical `/app/teams/:teamId/...` deep links (carried as a `route` query param — this app has no path router) for Home CTAs, refresh/Back/Forward restore, and pending-destination auth-resume. Independent of `API_DRIVEN_HOME` so routing and the new Home UI can be toggled separately during rollout |
