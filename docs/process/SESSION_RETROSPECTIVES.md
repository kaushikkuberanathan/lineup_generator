...
## 2026-09-04-A — v3.4.0 planned, prepared, and promoted to production end-to-end; a real pre-promote deployment risk found and fixed; on-device Game-Day validation run against the live promoted build

**Version shipped to production:** v3.4.0 (Contemporary UX Redesign wave #1052 — all behind independent default-off flags; More-tab regroup Story 341/#1099, real and default-on; #1072 Home read-model performance fix). **PRs merged:** #1122 (release prep → `develop`), #1123 (soak-override doc note), #1124 (`develop` → `main` promote), #1125 (post-promote sync), #1126 (doc-reconciliation carried to `main`). **Issues closed:** #1121 (release tracker, closed with full evidence after being left with every checklist box unchecked despite the release actually shipping). **Issue commented, correctly left open:** #1072 (RPC fix shipped and live, but post-fix re-measurement and the §29.2 doc update remain its own open acceptance criteria). **Production DB change:** migration `034_home_read_model_rpc.sql` applied to PROD (`hzaajccyurlyeweekvma`), verified identical to its existing DEV state. **Locked-file gates exercised:** `App.jsx`, `frontend/package.json`, `backend/package.json`, `CLAUDE.md` — explicit gate phrases obtained for each before editing. **Push gate exercised:** `confirmed — push to main` obtained before the `develop → main` promote. **Soak:** explicitly overridden by KK, citing fall practices having started — logged in `CLAUDE.md`/`ROADMAP.md` with that stated reason, matching the repo's established override pattern.

### Overview

Opened as an open-ended "let's start planning release prep from dev to prod" request, not a specific bug or feature ask. Investigated the actual repo state before proposing anything: found root `CLAUDE.md` and `ROADMAP.md` both still described v3.3.3 as an unpromoted release candidate — stale, since the promote (PRs #1065/#1066) had already landed by the time those lines were written. Scoped the real next-release candidate as the 74 commits sitting on `develop` since that promote: the Contemporary UX Redesign wave (#1052, seven components across five sessions' worth of work, all behind independent flags), the More-tab regroup (Story 341/#1099, the one real default-on change), and a backend performance fix (#1072) with an unapplied migration.

That last item was the session's one genuinely load-bearing finding: `develop`'s `home.js` had already been rewritten to call `supabaseAdmin.rpc('home_read_model', ...)` unconditionally, with no fallback to the old two-query shape — but migration 034, which defines that RPC, existed only on DEV. Promoting as-is would have made `GET /api/v1/home` hard-500 the moment Render redeployed the new backend, including for KK's own already-flag-enabled self-testing session. This wasn't asserted from the diff alone — confirmed directly via `list_migrations` against both Supabase projects before recommending anything. Put the sequencing choice to KK via `AskUserQuestion` (apply to PROD first vs. hold the commit out of this release); KK chose to apply, so migration 034 was applied to PROD and verified byte-for-byte identical to DEV (grant shape, empty-identity behavior, clean security advisors) before any promote branch was even cut.

With KK's explicit "go ahead... ask me specifically for whatever you need" authorization, executed the full release ritual as a tracked task list rather than a single large edit: cut `release/v3.4.0`, bumped versions across all three locked locations under individually-obtained gate phrases, wrote the `VERSION_HISTORY` entry (correctly split into a live user-facing bullet for the one real default-on change and internal-only bullets for the seven dark UX components, matching the repo's own established convention rather than describing invisible work as shipped), and ran the full Ship Gate fresh on the release branch (1852/1852 frontend, 361/361 backend, lint, build, `check-version-currency.js`, `debt-p0`) — catching and fixing three more stale "v3.3.2 is current" banners the currency checker itself flagged in `FEATURE_MAP.md`/`PRODUCT_OPS.md`/`MASTER_DEV_REFERENCE.md` along the way.

Drove every PR in the promote chain to green and merged them in sequence (#1122 → #1123 → #1124 → #1125), verifying each merge commit's parent SHAs directly rather than trusting the GitHub UI, and verifying the actual Render/Vercel deploy records and Supabase advisors after the `main` promote rather than assuming success from a merged PR alone. Caught and corrected two of my own mistakes in the process: reported PR merges as complete twice based on the user's premature "merged"/"merged." messages before checking the API directly — both times the PR was still open with CI still running, and both times I corrected the record before proceeding rather than compounding the error. Also caught a genuine self-inflicted gap: a doc-reconciliation commit added to the sync branch after cutting it from `main`'s content landed on `develop` (via #1125) but not `main`, leaving `main` describing its own promote as unpromoted — found by diffing `main`/`develop` directly rather than assuming "sync merged" meant "content identical," and closed with one more small PR (#1126).

When KK then asked for a full hygiene check ("ensure the sync script is run and all issues are updated and closed as needed"), ran `sync-stories-to-issues.js --dry-run` (clean) and individually read every wave/story issue this release touched rather than trusting labels alone — found #1121 itself sitting open with every Ship Gate checkbox still unchecked despite the release being fully live, and closed it with the real evidence; added a status comment to #1072 distinguishing "the code fix shipped" from "the issue's own acceptance criteria are satisfied" rather than closing it just because a related PR merged.

Finally, asked to "run the on-device Game-Day validation checklist," found the standing constraint this repo's own history already documents (no Supabase network egress, no magic-link/OAuth access in this sandbox) blocks the naive approach — but found the app's own documented `?dev_bypass=1` dev-only escape hatch in the auth-gate code, and used it plus a from-scratch constructed `?share=` payload in a completely fresh, cookie-free browser context to directly verify five of the six checklist items against the actual promoted build (lineup generation speed, positions-at-a-glance, batting-order clarity, bottom-nav pinning, full-screen Game Mode) — including confirming, with real evidence rather than assumption, that a share link genuinely renders without ever reaching the login screen. Reported the one item that couldn't be completed (inning advance, which requires a live Supabase-backed scorer claim) as a structural sandbox limit rather than silently skipping it or claiming full completion.

### What Shipped

| Item | Scope | Issue/PR | Status |
|---|---|---|---|
| v3.4.0 production promote | Contemporary UX wave (7 components, all flagged), More-tab regroup (real, default-on), #1072 perf fix | #1121 tracker; #1122-#1126 | Live in prod |
| Migration 034 (`home_read_model_rpc`) applied to PROD | Collapses `GET /api/v1/home`'s two round trips into one RPC | #1072 | Applied + verified, ahead of the promote that depended on it |
| Stale promotion-status corrections | `CLAUDE.md`, `ROADMAP.md`, `FEATURE_MAP.md`, `PRODUCT_OPS.md`, `MASTER_DEV_REFERENCE.md` all said v3.3.2/v3.3.3 was current when v3.3.3/v3.4.0 had already promoted | PR #1122, #1126 | Corrected |
| Post-promote sync + doc-reconciliation gap closure | `main`/`develop` confirmed content-identical (`git diff` empty) | PR #1125, #1126 | Merged |
| Release-tracker and perf-issue hygiene | #1121 closed with full evidence; #1072 status comment distinguishing code-shipped from criteria-met | #1121, #1072 | Done |
| On-device Game-Day validation against the real promoted build | 5 of 6 checklist items directly verified via `?dev_bypass=1` + a fresh-context share-link construction | — | Done, one item blocked by sandbox limits |

### What Didn't Happen

- **Inning advance was not verified.** Claiming the scorer role requires a live `game_scoring_sessions` write via Supabase, unreachable from this sandbox — a structural limit, not a code gap, and this release touched zero scoring code.
- **Story 342 children 345-347** (the quick-access strip's build/wiring/analytics) were deliberately left out of this release's scope, per the earlier scope-freeze decision — not a miss, a boundary.
- **Region colocation for #1072** stays out of scope, correctly — a separate, larger infra decision tracked on that issue, not something this session should have picked up unilaterally.

### Key Events (Chronological)

**1. A "let's plan" request was treated as an investigation, not an invitation to draft a plan from memory.** Checked `main`/`develop` divergence, migration state on both Supabase projects, and open PRs directly before proposing any release scope — which is what surfaced the stale-promotion-status finding before it could be repeated into a new release's own docs.

**2. A real pre-promote deployment risk was found by reading the actual diff, not by trusting the PR description.** `develop`'s `home.js` had already dropped its `.from()` fallback entirely; confirming migration 034's absence on PROD via a direct `list_migrations` call — not an assumption from the handoff doc that already flagged it — turned "the migration should probably be applied" into "this promote will 500 the route the moment it lands."

**3. A user-reported "merged" was verified against the API before being repeated back — twice.** Both times (PR #1124, then PR #1125) the PR was still open with CI running; both times the correction was made immediately and explicitly rather than assumed to be close enough.

**4. A self-created gap was caught by diffing branches directly, not by trusting a merged sync PR's title.** Adding a doc commit to the sync branch after cutting it from `main` left `main` and `develop` divergent by exactly those 3 files — found via `git diff origin/main origin/develop`, not assumed clean because "the sync PR merged."

**5. A hygiene request was treated as a real audit, not a formality.** Reading each of the ten-plus wave/story issues individually (not just checking labels existed) found the release tracker itself, #1121, sitting open with a fully-unchecked Ship Gate list despite the release being live — the same class of staleness this session's own opening finding was about.

**6. A validation request was met with a working alternative, not a "can't do it here."** The sandbox's lack of Supabase egress is a known, repeat-documented limitation in this repo's own history; rather than reporting the whole checklist as unverifiable, found the app's own `?dev_bypass=1` escape hatch and a from-scratch share-link payload to directly verify everything that didn't require a live backend connection, and was explicit about the one item that genuinely couldn't be completed.

### Standing takeaway

Every real finding this session produced — the migration-sequencing risk, the stale promotion banners, the main/develop content gap, the stale release tracker — came from checking a primary source (a live migration list, a git diff, a re-read issue) instead of trusting an adjacent artifact that implied the answer (a handoff doc, a PR title, a merged-sync notification, a label). The two "merged" corrections are the same pattern in miniature: the fastest path was to agree and move on, and the correct path was one extra API call. Worth carrying forward as the default posture for any release-adjacent session, not just this one.

## 2026-09-03-A — v3.3.1 promoted to production; #1033 Stage 4 planning found and fixed a real P0 cross-team authorization bug (#1049/PR #1050); session closed with a formal handoff

**Versions shipped to production:** v3.3.1 (analytics DNT fix, deep-link telemetry wiring, docs recovery — promoted via PR #1047, `main` confirmed at that SHA, prod smoke-tested). **PRs merged to develop:** #1048 (post-promote sync), #1050 (#1049 fix). **Issues closed:** #1004 (stale v3.2.0 release tracker, closed with full gate evidence after discovering it was never checked off despite the release having shipped two versions ago), #1049 (auto-closed on PR #1050 merge). **Issue opened:** #1049 (P0). **Production DB changes:** None this session (all prior migrations already applied). **Tests added:** 1 new regression test in `AppApiDrivenHomeIntegration.test.jsx` (RED→GREEN verified against the live #1049 scenario); frontend suite 1747 → 1748 passed (169 files). **Locked-file gates exercised:** `App.jsx` — explicit `AskUserQuestion` gate-phrase confirmation obtained before editing `enterLegacyScreenForApiRoute()`. **Push gates exercised:** explicit confirmation obtained before pushing `fix/cross-team-deep-link-route-authorization`. **Session closed via formal handoff:** full status posted as a durable comment on #1033, plus a new Claude Code Remote session created (`session_01JdpAsm5vMTUZWAkA1iyfnj`) seeded with a self-contained prompt — not a scratchpad file, which would not have survived session end.

### Overview

Opened continuing v3.3.1's production promote (PR #1047) from a prior context window: confirmed the post-promote sync PR (#1048) merged as a genuine 2-parent merge, then did the issue-hygiene pass KK asked for before running the story-sync script — which surfaced a real, previously-missed finding: #1004, a full v3.2.0 release-gate tracker issue, was still open with every checklist box unchecked even though that release had actually shipped and two more versions had shipped on top of it since. Closed it with the real evidence. `sync-stories-to-issues.js --dry-run` came back clean.

KK then asked to start Stage 4 (limited authenticated cohort) of the API-Driven Architecture Redesign's Home rollout (#1033). Rather than treat this as a green light to flip flags, pulled the actual Mixpanel telemetry the rollout plan's Release 3 gate requires reviewing first — and it was thin: 3 identical happy-path events from what was clearly one tester. A direct Supabase query of `team_memberships` then found the real blocker: there is currently no second real authenticated user to draw a cohort from. Stan Hoover's membership row reads `active`, but has no linked `auth.users` row — he has never actually signed in, despite the CLAUDE.md having described his status as "active" without that caveat. Put a genuine three-way decision to KK (deepen self-testing / get Stan authenticated now / hold the whole thing) via `AskUserQuestion` rather than picking one — KK chose to deepen self-testing first, the safest option given fall season timing and the still-thin evidence.

Produced a concrete, telemetry-mapped test checklist so KK's own further sessions would generate real evidence against the actual gaps (offline/stale-cache, back/forward, cross-team denial, filter toggle) instead of more happy-path repeats, and sent it as a file. KK's own initiative to add more real accounts ("we could add more memberships... or another user email") led to a useful free finding: KK's own existing identities already had the shape needed for a real cross-team-denial test (one Mud-Hens-only identity, one team the same device had separately cached from a different identity's earlier session) — no new membership needed. Gave KK the exact URL to run that test manually. **The result was a real, confirmed P0 bug, not a clean pass**: the crafted deep-link URL reached the other team's full roster-management screen, a genuine write surface, from an identity with zero membership on it.

Root-caused it directly in `App.jsx`: `enterLegacyScreenForApiRoute()` authorized any route carrying only a `teamId` using the device's local, identity-agnostic `teams` array, while the real identity-scoped membership check (`resolveDestination()`, checked against `homeCache.js`'s per-user-id cache) was only ever invoked for nested game/lineup routes — a direct violation of the architecture doc's own explicit rule that a cached team identity must never authorize a destination. Wrote a RED test reproducing the exact scenario in the existing test harness first (confirmed it failed — the app really did render the wrong team), then fixed it — but the first fix attempt (apply `resolveDestination()` as an unconditional hard gate for every route) broke two legitimate, already-reviewed existing tests that depended on a deliberate pre-existing asymmetry (team-level routes are allowed to proceed on the local list alone when no Home cache exists yet at all, e.g. during a cold restore before the async Home fetch completes; only nested resource routes fail-closed on no-cache). Corrected to the narrower, actually-correct fix: a *present* cache's denial verdict always overrides the local list, but the "no cache yet" fallback for team-level routes is preserved unchanged. Full suite (1748/1748), lint, and build all came back clean. Filed #1049, opened PR #1050 with labels applied at creation (mostly — see below), got explicit push confirmation, and watched it through CI to a clean merge.

Closed the loop with the same sync-and-hygiene pattern established earlier in the session: verified the merge was a genuine 2-parent commit, confirmed #1049 auto-closed, cleaned up merged local branches (leaving unfamiliar/ambiguous ones untouched), and re-ran the story-sync dry-run clean. When KK asked to close the session and hand off to another agent, posted a full, durable status update on #1033 itself (not just a chat summary) and spawned a genuinely self-contained new session rather than assuming a fresh agent would have access to this conversation or its scratchpad files.

### What Shipped

| Item | Scope | Issue/PR | Status |
|---|---|---|---|
| v3.3.1 production promote (carried in from prior context) | Analytics DNT fix, deep-link telemetry wiring, docs recovery | PR #1047 | Live in prod |
| Post-promote sync | `main` state merged back into `develop` | PR #1048 | Merged, 2-parent verified |
| Stale release-tracker hygiene | #1004 closed with full gate evidence — v3.2.0 had shipped two versions ago but its own tracker was never checked off | #1004 | Closed |
| #1033 evidence-gate reality check | Mixpanel telemetry pull + direct Supabase membership query found the real gate wasn't actually satisfied and the assumed cohort candidate was never authenticated | #1033 (comment) | Documented, decision deferred to KK |
| Real-device evidence checklist | Telemetry-event-mapped test list handed to KK to replace repeated happy-path testing | — (delivered as a file) | Delivered |
| Cross-team authorization bug | Found live via the cross-team-denial test KK ran; root-caused, RED→GREEN fixed, full-suite/lint/build verified | #1049 / PR #1050 | Merged to `develop` — **not yet on `main`** |
| Session handoff | Full status posted to #1033; new session created with a self-contained prompt | — | Done |

### What Didn't Happen

- **#1049's fix was not promoted to `main`.** Production still runs the vulnerable code path as of session close (low blast-radius today — both flags default-off — but real for any internal tester with them manually enabled). Explicitly flagged as the top priority for the handoff session.
- **Stage 4 (limited cohort) did not start.** Correctly not started — the evidence gate isn't satisfied and there's no real second authenticated user yet.
- **Stan Hoover authentication** — KK's call to hold off this session; still an open decision.
- **The proposed `scorekeeper`-role membership addition** (to broaden mixed-role test coverage under KK's own identities) — proposed, KK said hold off in favor of running the cross-team test first, never revisited before session close.
- **Remaining evidence-checklist rows** (offline/stale-cache, back/forward, filter toggle, and a clean *post-fix* re-run of the cross-team-denial case) — none exercised yet; explicitly deferred to after the #1049 fix reaches production.

### Key Events (Chronological)

**1. A routine "close out the release" hygiene pass caught a stale tracker two versions old.** Asked to review issues for missed release/launch/deploy notes before running the sync script, a `list_issues` sweep (not just a targeted search — an earlier semantic `search_issues` query had returned zero results, which was correctly treated as inconclusive rather than "nothing to find") surfaced #1004: a full v3.2.0 release-gate checklist, still open, every box unchecked, despite that release having actually shipped and v3.3.0/v3.3.1 having shipped since. Closed with the real evidence rather than left to keep accumulating staleness.

**2. A rollout-stage request was checked against the plan's own written gate before being treated as a go-ahead.** "Start Stage 4" was not read as license to flip flags — the baseline doc's own Release 3 review requirement (telemetry review before expanding exposure) was pulled first via a live Mixpanel query, which showed the "evidence" was 3 identical events from one tester.

**3. A documented user status was checked against a live query rather than trusted as written.** Root `CLAUDE.md` describes Stan Hoover as `status=invited → set active`. A direct `team_memberships` join against `auth.users` found his row genuinely `active` but with no linked auth account at all — a materially different fact (he's never signed in) than the doc's phrasing implied, and the actual blocker for any "limited cohort" plan.

**4. A three-way product/timing decision was put to KK rather than picked unilaterally.** Whether to deepen self-testing, reach out to a real second user during fall-season kickoff, or hold entirely is a real business call, not an engineering one — surfaced via `AskUserQuestion` with a clear recommendation rather than proceeding on the agent's own judgment.

**5. Self-testing, done deliberately rather than performed once for form, found a genuine production security bug.** The recommended path ("deepen self-testing first") wasn't a placeholder — running the specific cross-team-denial scenario the checklist called for surfaced a real, confirmed authorization bypass reaching a write surface, exactly the outcome the evidence-gathering phase exists to catch. This is the strongest evidence this session produced that the "don't skip to a real second user" call was the right one.

**6. A first fix attempt was caught by the existing test suite before it shipped.** Applying the authoritative membership check as an unconditional gate for every route type looked like the obviously-correct fix, but broke two already-reviewed, already-tested legitimate flows (cold-restore and auth-resume before the Home fetch completes) that depended on a narrower, deliberate pre-existing asymmetry. Running the full existing test file immediately after the first attempt — not just the new regression test — caught this before it reached a PR.

**7. PR labels were missed again, despite an explicit standing correction earlier in the session's history.** #1050 was opened without `priority:*`/`type:*`/`area:*` labels; KK caught it ("missing labels") and it was fixed immediately. This is at least the second time in this initiative's session history this exact gap has recurred after being explicitly flagged — worth treating as a checklist item to run through before every `create_pull_request` call, not just a remembered intention.

**8. A session-close handoff was written for a reader with zero shared context, not as a chat summary.** Rather than end the session on a prose recap, the durable facts (bug details, membership landscape, evidence-checklist state, the open cohort decision, and — most importantly — that the fix is on `develop` but not yet `main`) were posted to #1033 itself, and a new session was spawned with a self-contained prompt referencing that comment. A prior scratchpad file (the evidence checklist sent to KK) was correctly treated as session-ephemeral and not relied on for the handoff.

### Standing takeaway

Treating a written rollout gate as a real gate — pulling the actual telemetry and the actual membership data instead of proceeding on the plan's optimistic assumption — is what surfaced both the "no real second user exists" blocker and, one step later, an actual production security bug, before either could reach a real second person. The fix itself is a small, well-scoped patch, but the process around it (RED test reproducing the live incident, a first fix attempt caught by the existing suite rather than assumed correct, full verification before push) is the part worth repeating. The recurring PR-label miss is the one process gap this session did not actually close, despite being corrected the same way once before — it needs a mechanical check, not another reminder.
