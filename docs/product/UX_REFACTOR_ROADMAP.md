...
**Created:** 2026-05-01
**Status (reviewed 2026-08-23):** Foundation shipped in v2.5.6; UI primitives and early consumer migrations shipped through v2.5.14; Phase 3 call-site/primitives migration is complete through v2.8.4; Phase 4 legacy `C.*` color retirement is complete through v2.8.5 with zero `C.*` references remaining in `App.jsx`. Phase 5 auth token convergence shipped on `develop` with the v2.11.0 work; Phase 6 design-system documentation remains proposed. Historical per-step ledger entries below preserve their at-the-time branch/PR wording.
