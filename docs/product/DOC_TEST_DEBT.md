...
### August 6, 2026 — #428 definitively closed: 004_rls_fixes.sql confirmed fully applied to prod (both halves)

- ✅ **P1 — 004_rls_fixes.sql precondition status (#428)** — Resolved via direct, read-only verification against prod, closing the ambiguity between the migration file's own STOP banner and root `CLAUDE.md`'s "Phase 4 Cutover" section. Both were last touched 2026-07-31 on partial evidence (SELECT-side reads only); this pass ran the two SELECT-only checks that evidence explicitly deferred.
  - **Tooling note:** the `rls_test_anon_grants` RPC (migration 013) still does not exist in prod (confirmed absent again this pass — same `PGRST202` gap noted 2026-08-01 in this ledger's #428 follow-up entry above). Rather than re-run the gitignored `spike-428-teams-roster-probe.js` substitute, this pass used `supabase link --project-ref hzaajccyurlyeweekvma` + `supabase db query --linked` (Management-API-backed, direct Postgres access, no RPC needed) — a stronger, more direct read than either prior substitute.
  - **Check 1 (policies)** — `SELECT schemaname, tablename, policyname, permissive, roles, cmd FROM pg_policies WHERE schemaname='public' ORDER BY tablename, policyname` (the exact query already in `004_rls_fixes.sql`'s own header). Result: `teams`, `team_data`, and `roster_snapshots` carry exactly the policies 004 defines — `teams_auth_{select,insert,update,delete}`, `team_data_auth_{select,insert,update}`, `roster_snapshots_auth_{select,insert}` — all scoped `TO authenticated`, nothing else. Matches 004's designed end-state exactly.
  - **Check 2 (grants — the half #428 flagged as never independently confirmed)** — `SELECT table_name, grantee, privilege_type FROM information_schema.role_table_grants WHERE table_schema='public' AND table_name IN ('team_data','teams','roster_snapshots') AND grantee IN ('anon','authenticated') AND privilege_type IN ('TRUNCATE','DELETE')`. Result: **exactly two rows** — `anon:DELETE` and `authenticated:DELETE` on `teams` only. This is precisely 004 §5's documented deliberate exception (teams keeps DELETE pending the service-role delete-team endpoint; TRUNCATE revoked everywhere; DELETE revoked on `team_data`/`roster_snapshots`). Zero unexpected rows — no gap, no drift.
  - **Verdict:** 004_rls_fixes.sql is fully and correctly applied to prod, in its entirety, exactly as designed — not partially, not with an unnoticed gap. This is a **confirmation of the existing hypothesis**, not a new finding — no security anomaly surfaced.
  - **Follow-up required, not done in this pass:** root `CLAUDE.md`'s "Phase 4 Cutover" section is a locked file and no gate phrase was granted this session — its stale "do NOT run `backend/migrations/004_rls_fixes.sql` until cutover" line and its "not verified — deliberately not probed" framing around the grant-revocation half are now both incorrect and need a follow-up edit once the gate phrase is available. `backend/migrations/004_rls_fixes.sql`'s own header was updated this pass (not locked) with the definitive status; `CLAUDE.md` itself was deliberately left untouched.
  - Branch: `feature/428-rls-prod-verification`. Issue: [#428](https://github.com/kaushikkuberanathan/lineup_generator/issues/428).
