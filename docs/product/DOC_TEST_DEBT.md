...
### August 6, 2026 — #561: createTeam() second-team RLS bug — TWO stacked fixes, not one

- ✅ **P1 — createTeam() second-team writes silently RLS-denied (#561)** — Resolved, but root-cause investigation surfaced a SECOND bug the original issue didn't describe.
  - **Bug #1 (not in the original issue, found investigating it):** `dbSaveTeams()` (frontend/src/supabase.js) used `.upsert(rows, { onConflict: 'id' })`. Postgres enforces the UPDATE policy's WITH CHECK for `INSERT ... ON CONFLICT DO UPDATE` **even when no conflict occurs** (documented Postgres RLS behavior — confirmed empirically against the real DEV project: a plain `.insert()` for a brand-new row succeeded, the identical row via `.upsert(..., {onConflict})` was RLS-denied with `42501`). `teams_auth_update` requires an existing active admin/coach membership; a brand-new team never has one. Net effect: this was unconditionally RLS-denied for **every** self-serve team creation, not only an existing coach's *additional* team as #561 scoped it. Fixed by rewriting `dbSaveTeams()` to a plain INSERT with a fallback UPDATE only on a real conflict (`error.code === '23505'`) — never invokes the ON CONFLICT DO UPDATE construct. No App.jsx call-site change needed (all three callers — createTeam(), the edit-team flow, and the migration/hydration path — keep the same signature and behavior contract).
  - **Bug #2 (the original report):** even once the teams row exists, no code path ever wrote a `team_memberships` row for the creator, so the first `team_data` save hit `team_data_auth_insert`'s WITH CHECK (same admin/coach + active membership requirement) and was silently RLS-denied. Fixed via migration 018 (`handle_new_team` — a `SECURITY DEFINER` `AFTER INSERT` trigger on `public.teams`) auto-provisioning `role=admin`/`status=active` membership for `auth.uid()` (NOT `teams.owner_id`, which is always `''` — never populated by any known write path) whenever an authenticated session inserts a new team row.
  - **Verification:** RED confirmed first — `npm run test:rls` against DEV (psqvzppphdedqkpmarwx) before either fix landed reproduced both failures exactly (`42501` on `teams` for the upsert shape, `42501` on `team_data` for the missing-membership case). Migration 018 applied to DEV directly (not prod — tonight's hard constraint); full RLS suite re-run GREEN: 35/35 non-skipped (7 pre-existing #355 skips untouched), including the new TM1-TM4 suite (`backend/src/__tests__/rls/teamMembershipAutoProvision.test.js`). Frontend: full suite 1023 passed/1 skipped (85/85 files), `npm run build` clean, `dbSaveTeams.test.js` fully rewritten for the new insert-then-fallback mock shape (7 cases, all passing). Backend unit suite 111/111 unaffected (no route code touched).
  - **NOT done in this PR, flagged for KK:** migration 018 is drafted and DEV-verified only — **not applied to prod**, per tonight's no-prod-writes constraint; needs a deliberate prod-apply step in a future session, same "commit first, apply later" convention as every other migration in this tree. Also: existing already-broken teams (created before this fix ships) **cannot be backfilled** — `teams.owner_id` is always `''`, so there is no stored data identifying who created an already-broken membership-less team row. Recovering those requires a manual admin action per affected team once the correct owner is identified out-of-band. A prod audit query (count of teams with zero `team_memberships` rows) would size this list but was not run as part of this PR.
  - Branch: `fix/561-createteam-membership-rls`. Issue: [#561](https://github.com/kaushikkuberanathan/lineup_generator/issues/561).
