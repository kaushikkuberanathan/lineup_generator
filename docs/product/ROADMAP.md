...
> Last updated: 2026-09-04 (v3.4.0 promoted to `main`, live in production — PR #1124 merged `develop` → `main` as `e37cb06`; release tracker [#1121](https://github.com/kaushikkuberanathan/lineup_generator/issues/1121)). Previously updated 2026-09-04 (v3.4.0 release preparation; scope frozen at `develop` tip `8384f24` against production v3.3.3 `a1b916a`). Previously updated 2026-09-04 (v3.3.3 promoted to `main`, live in production — PR #1065 merged `release/v3.3.3` → `main` as `d2f88bb`, then PR #1066 merged `develop` → `main` directly as `a1b916a` to absorb the remaining drift; this line previously described v3.3.3 as an unpromoted release candidate, which was stale by the time it was written — corrected here). Previously updated 2026-09-03 (v3.3.2 promoted to `main`, live in production — PR #1054, merge commit `162061c`, confirmed genuine 2-parent; post-promote sync PR #1055 merged as `354e707`, `main`/`develop` confirmed content-identical). Previously updated 2026-09-02 (v3.3.0 release preparation; scope is full develop tip `b9215dd` against production v3.2.0 `aa519bf`, no dedicated tracker issue); 2026-09-01 (v3.2.0 release preparation; scope frozen at `e79487d`, release tracker #1004).
> MVP launched: March 24, 2026

## 🚧 Contemporary UX Wave E — Unauthenticated Share View

Issue [#1092](https://github.com/kaushikkuberanathan/lineup_generator/issues/1092) adds a reversible, default-off `UX_SHARE` treatment to the public lineup screen. It preserves the navy header and full-color Dugout Lineup mark, uses gold for primary selection/print actions, keeps meaningful field-position and genuine error colors, and applies the shared contemporary typography, icon, card, status, and segmented-control contracts.

Both Supabase-backed `?s=` links and legacy `?share=` payload links remain above the authentication gate. Viewer-mode routing, payload construction, first-name display, absence handling, batting and walk-up-song content, print behavior, and detailed malformed/inaccessible/timeout/not-found copy remain unchanged. New coverage protects the flag-off fallback, anonymous direct routing, loading and unavailable states, empty roster/batting states, mobile touch sizing, and print invocation/CSS. Validation on the feature branch: RED mutation failed all four new App selectors as intended; focused suite 80/80; full frontend suite 1,862/1,862 across 187 files; lint and production build clean; direct/reload browser checks at 375px and 393px showed exact containment, no Vite overlay, and no red CTA/background treatment. The print action and print-only CSS media contract are verified; native print-preview acceptance remains part of PR preview review.

## 🚧 Contemporary UX Wave F — Authentication, access, and system states

Issue [#1148](https://github.com/kaushikkuberanathan/lineup_generator/issues/1148) introduces reusable `AuthWorkspace` and `SystemStateScreen` compositions behind independent default-off `UX_AUTH` and `UX_SYSTEM_STATES`. Login, Request Access, Pending Approval, No Membership, auth loading, and Maintenance consume the shared calm cream/navy/gold hierarchy; OfflineIndicator remains non-interactive and preserves its meaningful connectivity colors. Magic-link and Google OAuth behavior, role normalization, consent/version capture, team preselection, preserve-session submission, auth routing, maintenance precedence, and public Share/Game Mode access are unchanged. The migration also corrects missing programmatic label associations on Request Access first name, last name, email, and team ID fields.

Validation on the feature branch: RED mutation failed all three new App flag-integration selectors as intended; focused suite 118/118; full frontend suite 1,875/1,875 across 189 files; lint and production build clean. Browser QA at 375x812 confirmed exact horizontal containment, labeled controls, 44px primary/secondary actions, and the intended cream/navy/gold hierarchy across Login, Request Access, and Maintenance.

## ✅ v3.4.0 PROMOTED AND LIVE IN PROD — Contemporary UX wave, More-tab regroup, and Home read-model performance fix
...
**Merged to `develop` 2026-09-04** (PR [#1122](https://github.com/kaushikkuberanathan/lineup_generator/pull/1122), regular merge, `5a9d494`) — confirmed a genuine 2-parent merge (parents `8384f24` prior `develop` + `fd12bb1` release branch tip). All CI checks green before merge (Frontend Tests, Backend Unit + Integration, RLS Policy Suite, CodeQL, sync-script, PR-title-direction guard), Vercel preview deployed clean, `mergeable_state: clean`. A follow-up soak-override doc note (PR [#1123](https://github.com/kaushikkuberanathan/lineup_generator/pull/1123), `42545bc`) merged the same session. **Soak explicitly overridden 2026-09-04 by KK, citing fall practices having started** — same "fall season readiness" pattern as v2.9.0 through v3.3.0's overrides; `develop`'s soak-lock branch protection is already off (honor-system only, per the cross-track note in `CLAUDE.md`), so this override doesn't require unchecking anything, just proceeding straight to the `main` promote.

**Promoted to `main` 2026-09-04** (PR [#1124](https://github.com/kaushikkuberanathan/lineup_generator/pull/1124), regular merge, `e37cb06`) — confirmed a genuine 2-parent merge (parents `a1b916a` prior `main` + `42545bc` `develop` tip). Every CI check green before merge (Backend Unit/Integration, RLS Policy Suite, CodeQL ×2, both Analyze jobs, PR-title-direction guard, merge-policy squash-detection guard). **Prod smoke test confirmed same session, verified directly via the Render/Vercel platform APIs:** Render backend (`lineup-generator-backend`, `srv-d70lkr9r0fns73cufidg`) deploy `dep-dadkcju7bikc73cm8i0g` for `e37cb06` status `live`; Vercel (`line-up-generator`) production deployment `dpl_7nmK5v5RzVVDo5xd7CWg918hSLoM` `READY`, `githubCommitSha` matching exactly, `target: production`; Supabase (`hzaajccyurlyeweekvma`) security advisors clean (same 4 pre-existing INFO/WARN findings as before the promote, nothing new). Migration `034_home_read_model_rpc` reconfirmed present on both DEV (`20260904104942`) and PROD (`20260904213430`) directly before this promote. Post-promote sync (PR [#1125](https://github.com/kaushikkuberanathan/lineup_generator/pull/1125)) merged the same session — pure ancestry-linking, no real content diff since `develop` was exactly one commit behind `main` (the promote merge commit itself).
...
**Patch-version rationale:** two real bug fixes (both affecting every user, not gated by either API-driven-Home flag) plus a CI tooling addition and a docs correction — no new user-facing capability, matching this repo's patch-vs-minor convention.

**Included scope:**
- [#1060](https://github.com/kaushikkuberanathan/lineup_generator/issues/1060) (P1) fixed via PR [#1061](https://github.com/kaushikkuberanathan/lineup_generator/pull/1061): `useAuth.js`'s `checkSession()` and `onAuthStateChange`'s `SIGNED_IN` handler both validated an already-confirmed local Supabase session against the backend via a fetch to `/api/v1/auth/me`. A thrown fetch (network unreachable, e.g. offline) fell into the same catch-all as a genuine backend rejection, logging a user out purely because the network was down. Found during #1033 Stage 3/4 offline-evidence testing. RED→GREEN verified (new tests A4b/B5 in `auth.test.js`).
- [#1062](https://github.com/kaushikkuberanathan/lineup_generator/issues/1062) (P2) fixed via PR [#1064](https://github.com/kaushikkuberanathan/lineup_generator/pull/1064): `navigator.onLine`/the browser's `online`/`offline` events only reflect adapter-level state changes, not "requests are actually succeeding" — confirmed live during #1033 evidence-gathering (DevTools Network throttled to Offline blocked every request but never flipped `navigator.onLine`, so `useHomeScreen.load()` kept attempting and retrying real fetches instead of the offline cache-only short-circuit that code already tries to give). New `frontend/src/utils/networkHealth.js`: a self-correcting signal — two consecutive reported network-level failures flip it to offline, any successful response or real browser event flips it back immediately. Wired into `client.js`'s retry loop, `useAuth.js`'s two `#1060` network-failure catches, and `App.jsx`'s `isOnline` state (the last edited under its own literal gate phrase). RED→GREEN verified via mutation for every wired call site.
- `scripts/check-version-currency.js` + CI wiring, PR [#1059](https://github.com/kaushikkuberanathan/lineup_generator/pull/1059): automated check against the exact version-staleness recurrence documented elsewhere in this file's own history. Landed on `develop` 2026-09-03 but deliberately not promoted that session ("no that can remain in develop" — KK); rides along with this batch now per KK's explicit call to promote "whatever else is in dev."
- [#1063](https://github.com/kaushikkuberanathan/lineup_generator/pull/1063): corrected root `CLAUDE.md`'s stale "backend dev deleted April 27, 2026" claim — found during #1033 evidence-gathering when a Supabase write meant for `dev.dugoutlineup.com` silently went to the wrong (production) project instead, because the doc's claim that no dev backend existed was itself wrong.
- Both `API_DRIVEN_HOME`/`API_DRIVEN_ROUTES` remain default-off in production — zero live behavior change for any general user from any of these four items beyond the two real bug fixes, which are not gated by either flag and affect every user.

**Ship Gate status:** frontend 1759/1759 across 169 files (+11/+1 vs. v3.3.2's 1748/168 — networkHealth.test.js new (6), api.client.test.js +3, auth.test.js +4 assertions across existing tests, check-version-currency.test.js's own suite runs separately via the `sync-script` CI job not this count), backend unit 361/361 (unchanged — no backend code touched by this batch), lint clean, production build clean — all directly re-verified on a fresh `develop` checkout during release prep.
...
- API-Driven Architecture Redesign, Phase 0 + Phase 1 (initiative [#1012](https://github.com/kaushikkuberanathan/lineup_generator/issues/1012), baseline doc [`docs/product/API_DRIVEN_ARCHITECTURE_REDESIGN.md`](API_DRIVEN_ARCHITECTURE_REDESIGN.md)) through PRs #1034/#1035/#1036 — **entirely behind `API_DRIVEN_HOME`/`API_DRIVEN_ROUTES`, both default off, zero live behavior change:**
  ...
  - **Post-promote, #1033 Stage 3 (internal cohort) performed 2026-09-03:** — real prod session (KK's account, admin on Mud Hens + a new coach membership granted on the existing “Bananas” team for a genuine 2-team test, per #1033's cohort requirement), both flags enabled per-device. Backend confirmed 2 real `GET /api/v1/home` calls (200, teamCount:2, incl. a direct `/app/teams/:teamId/roster` deep link). Found and fixed along the way: [#1041](https://github.com/kaushikkuberanathan/lineup_generator/issues/1041) — Mixpanel silently dropped every event (not just `home_*`) for any browser sending Do Not Track, discovered because zero `home_*` events reached Mixpanel despite confirmed-successful backend calls; fixed by overriding `ignore_dnt` and disclosing it in a new Privacy Policy version (1.1). Stage 4 (limited authenticated cohort) not yet started.
  - **Stage 4 blocked on a develop → main promote, discovered 2026-09-03:** the #1041 DNT fix (`f5d489b`), deep-link telemetry wiring (PR #1043), and the retro-doc recovery (PR #1044) all merged to `develop` only — `main` is still at v3.3.0 (`dbaa769`). Confirmed live: a real-device test against production (`dugoutlineup.com`) generated genuine `GET /api/v1/home` traffic (backend, which deploys from `main` and was unaffected by these changes, responded correctly, including a 304 revalidation), but zero Mixpanel events reached the account — because the deployed frontend bundle still runs `ignore_dnt: false`, not yet the fix. KK's explicit call: wait for the next planned promote rather than force one early. Stage 4's telemetry-review requirement stays blocked until then.

**Ship Gate status:** frontend 1745/1745 across 168 files, backend unit 361/361, lint clean, production build clean, `debt-p0` gate clear (0 open P0 items) — all directly re-verified on a fresh `develop` checkout during release prep, not carried forward. Golden-path coverage exists for every touched feature (Schedule, My Team/Roster player-profile routing, and the entire API-driven Home surface). `FEATURE_MAP.md`, `DOC_TEST_DEBT.md`, and `SOLUTION_DESIGN.md` are current as of this entry. **Promoted to `main` 2026-09-02** (PR [#1039](https://github.com/kaushikkuberanathan/lineup_generator/pull/1039), regular merge, `dbaa769`) — confirmed a genuine 2-parent merge. Prod smoke test same session confirmed clean; post-promote sync (PR [#1040](https://github.com/kaushikkuberanathan/lineup_generator/pull/1040)) merged same session. Long since superseded by v3.3.1 and v3.3.2's promotes.
