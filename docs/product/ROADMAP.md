...
- `docs/analytics/ANALYTICS.md` doesn't yet list `tos_consented`/`tos_link_opened`.
...
- **Migration 028 not yet applied to DEV or PROD, but `develop` already auto-deploys live** — `lineup-generator-dev-backend` (Render) auto-deploys from `develop` on every push, and `dev.dugoutlineup.com` (Vercel) does the same for the frontend. That means `POST /api/v1/auth/consent` is reachable on the live DEV backend right now, and every call to it currently 500s (`legal_consents` doesn't exist yet in the DEV database) — silent to the coach (`logLegalConsent()` swallows all errors client-side), but a real logged 500 in DEV's backend logs on every registration submission since this merged. Not a data-loss risk (nothing is silently dropped that would otherwise be saved — the table doesn't exist to write to), but worth applying migration 028 to DEV promptly so DEV stops generating spurious 500s.
