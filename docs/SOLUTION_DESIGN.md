...
**Known operational gap as of this writing:** migration 028 has not been applied to DEV or PROD, but `develop` already auto-deploys live to `lineup-generator-dev-backend` (Render) and `dev.dugoutlineup.com` (Vercel) on every push. `POST /api/v1/auth/consent` is therefore reachable on the live DEV backend right now and 500s on every call (`legal_consents` doesn't exist yet) — silent to the coach, a real logged 500 in DEV's backend logs. Not a data-loss risk (there is no table to write into yet), but worth applying the migration to DEV promptly.
