...
## Current Version
**v3.1.1 release candidate** — August 2026, on local release branch `feature/release-v3.1.1`, not yet pushed or promoted. Release tracker: #989. Scope is frozen at `develop` commit `34ff2b2` against production v3.1.0 merge `02abfc0`. This patch release contains database permission defense in depth, public activity-note cleanup, targeted regression coverage, and documentation reconciliation; it adds no new coach-facing capability. Open PRs #874/#892 and the unmerged Phase 4C scoring-security work are excluded. Final local verification, release-prep PR CI, DEV soak, real-device validation, and explicit `confirmed — push to main` approval remain open. Full version history lives in `frontend/src/data/versionHistory.js`.

- v3.1.1 (2026-08-31, **release candidate, local only**): migration 032 security hardening and RLS coverage (PR #970); QA coverage for player mapping, logout, auth rejection diagnostics, Songs, PWA install, demo-team loading, Roster, coach-PIN unlock, and Bottom Nav (PRs #971-#976/#980); coach-readable product-activity cleanup (PR #978); Phase 4C status and production-documentation reconciliation (PRs #979/#988). Patch bump: no new coach-facing capability. Bench fairness #942/#952 already shipped in v3.1.0. Tracker #989 remains open through production verification and post-promote sync.
