...
## Test Suite
Changes to `lineupEngineV2.js`, `scoringEngine.js`, or `playerMapper.js` → must pass frontend `npm test` (Vitest, 1422 passing / 1 skipped as of 2026-08-29 — fewer may be observed locally on Windows due to Bug #7 cold-start flake). Backend unit suite is 276/276 (see `backend/CLAUDE.md` § Test Suite for that count's own history). This line had sat at the stale v2.12.0-era 1090 figure through multiple subsequent releases — flagged as a carry-forward in the 2026-08-24-B and 2026-08-26-A session retros before finally being corrected in v2.15.0-prep (1368/1, 2026-08-26); corrected again for PR #867's `FAQSection.test.jsx` growth (6→15), again for v3.0.0's own promote (1377→1401 frontend; backend unit 254→269), and once more the same day after PR #907 (Terms of Service experience — separate work, unrelated to v3.0.0) added further coverage on `develop` (1401→1422 frontend, 122→124 files; backend 269→276).
...
## Current Version
**v3.0.0** — August 2026, **promoted and live in prod** (PR #903, merge `c865d4e`; post-promote sync PR #905, `0d24c30`). Verified live via the Render/Vercel platform APIs (backend deploy `dep-da9c3irl550s739rlc6g` status `live`, Vercel production deployment `READY`, both on `c865d4e`) plus **KK's confirmed real-device phone smoke test**. Soak was explicitly overridden by KK (~9h40m into the 24h window, 2026-08-29T11:07:18Z) — see below. This is a deliberate MAJOR bump — see the v3.0.0 entry immediately below for the explicit rationale. Full version history in `VERSION_HISTORY` constant in `frontend/src/data/versionHistory.js`.
