...
## Current Version
**v3.2.0 release candidate — local release-prep branch, not yet promoted:** release tracker #1004. Scope is frozen at `develop` commit `e79487d` against production v3.1.1 merge `a6a7c5a`. Included: PR #994 scoring RLS lockdown (migrations 033+031 already applied and verified on DEV+PROD), PR #995 analytics/stale-pitch maintenance, PR #1001 My Team dashboard/player profiles/primary Schedule navigation, and PR #1003's v3.1.1 status correction. Local release validation: frontend 1550/1550 across 146 files, backend unit 299/299 across 59 suites, lint clean, production build passed, and `debt-p0` clear. Open PR #892, remaining #355 cleanup, and later `develop` commits are excluded unless the frozen boundary is explicitly reopened. Promotion remains blocked on release-branch CI, real-device preview validation, a fresh 24-hour `develop` soak after this prep merges, the Ship Gate review, and literal `confirmed — push to main` authorization.

**v3.1.1** — **promoted to `main`, live in production** as of 2026-08-31 (PR #991, merge commit `a6a7c5a`, "release: promote v3.1.1 to production", confirmed genuine 2-parent merge — parents `02abfc0` (prior `main`) + `2867b95` (`develop` tip)). **Corrected 2026-09-01 — this line previously read "release candidate... not yet pushed or promoted" for a full day after it actually promoted; caught during a routine `gh` CLI auth-troubleshooting session that happened to diff `origin/main` against the frozen scope commit.** Release tracker: #989. Scope was frozen at `develop` commit `34ff2b2` against production v3.1.0 merge `02abfc0`, landed on `develop` via PR #990 (`chore(release): prepare v3.1.1 candidate #989`, `c78ac54`). This patch release contains database permission defense in depth, public activity-note cleanup, targeted regression coverage, and documentation reconciliation; it adds no new coach-facing capability. Open PRs #874/#892 remain excluded, as does the unmerged Phase 4C scoring-security work — see the next paragraph, since part of that work has since merged to `develop` (not `main`). Post-promote sync (PR #992) confirmed `main`/`develop` content-identical immediately after. Full version history lives in `frontend/src/data/versionHistory.js`.

**Not included in v3.1.1 — staged for the next release, `develop` only as of 2026-09-01:** PR [#994](https://github.com/kaushikkuberanathan/lineup_generator/pull/994) (#355 live-scoring RLS lockdown, migrations 033+031 applied to real DEV+PROD — see `docs/product/AUTH_SECURITY_AUDIT_ROADMAP.md` and the Known Open Bugs row 4 below) and PR [#995](https://github.com/kaushikkuberanathan/lineup_generator/pull/995) (`feature/maintenance-wave-2`) both merged to `develop` *after* the v3.1.1 promote — neither is in `main` yet. `develop` is 9 commits ahead of `main` as of this note. KK confirmed 2026-09-01 that #994 will ship on the next scheduled `develop`→`main` release, not as a standalone hotfix.
