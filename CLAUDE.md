...
## Test Suite
Changes to `lineupEngineV2.js`, `scoringEngine.js`, or `playerMapper.js` → must pass frontend `npm test` (Vitest, **1550 passing across 146 files** on the local v3.2.0 release-prep branch, 2026-09-01 — fewer may be observed locally on Windows due to Bug #7 cold-start flake). Backend unit suite is **299/299** (see `backend/CLAUDE.md` § Test Suite).
...
| 4 | **Phase 4C deferred** | **Updated 2026-08-31 — was stale.** Steps 1-5 of 7 are now done. Steps 1-2 (Story 129/#688): migration 019 Section A applied to PROD, auth-testing shims removed from `useLiveScoring.js`/`DugoutView.jsx`. **Steps 3-5 done 2026-08-31**: a real production game-day soak was confirmed complete (real authenticated coach, post shim-removal); migrations `033_scoring_rls_lockdown_section_b.sql` (drops the four `*_anon_test` backdoors + three `allow_scorer_writes` catch-alls) and `031_scoring_grant_revocation.sql` (revokes the underlying table GRANTs) were applied to **both DEV and PROD**; `LS1`-`LS7` in `policies.test.js` are un-skipped on branch `fix/355-scoring-rls-lockdown` (not yet CI-confirmed against a real Postgres instance — pending that branch's PR to `develop`). **The core exploitable vulnerability is fixed on both databases** — `anon` now holds zero privileges on all four scoring tables (verified via `pg_policies`/`role_table_grants` before/after on both DEV and PROD, live anon-write rejection probes, and a real end-to-end game scored on PROD immediately after the apply, confirmed via `scoring_audit_log` rows attributed to a real `auth.users` UUID). **2 steps remain** (6: restore `scorer_user_id`/`actor_user_id`/`recorded_by_id` from `TEXT` to `uuid`+FK; 7: remove the dead `isAdminTestMode` variable) — both deliberately out of scope for the 2026-08-31 apply, tracked in [#355](https://github.com/kaushikkuberanathan/lineup_generator/issues/355), which stays open for them plus one loose end: the four `public_read_*` SELECT policies were left in place (not dropped as originally planned in the proposal doc) pending a separate KK confirmation — functionally inert now that the underlying GRANT is revoked, but not yet cleaned up or reconfirmed. Full verification detail: `backend/CLAUDE.md` → Migration Notes, `backend/migrations/README.md` rows for `033`/`031`. |
...
## Current Version
**v3.3.2** — **promoted to `main`, live in production** as of 2026-09-03 (PR [#1054](https://github.com/kaushikkuberanathan/lineup_generator/pull/1054), merge commit `162061c`, "release: promote v3.3.2 to production", confirmed genuine 2-parent merge — parents `81745cd7` (prior `main`, v3.3.1) + `09832b6` (`develop` tip)). Single-issue security patch closing [#1049](https://github.com/kaushikkuberanathan/lineup_generator/issues/1049) (P0 cross-team deep-link route authorization bug), fixed via PR [#1050](https://github.com/kaushikkuberanathan/lineup_generator/pull/1050) and released via PR [#1053](https://github.com/kaushikkuberanathan/lineup_generator/pull/1053) (`release: prepare v3.3.2`, merge `09832b6`). Merged directly by KK — no release tracker issue (single-issue patch, not a batched pass). **Soak explicitly overridden by KK (2026-09-03)** — the fix had only just merged to `develop` when the decision was made to proceed rather than wait, on the grounds that KK himself was the party currently exposed to this exact P0 auth bypass on production (flags manually enabled for #1033 internal testing); the literal `confirmed — push to main` gate phrase was provided before the merge. **Verified directly after promote, not just asserted:** Render backend (`lineup-generator-backend`, `srv-d70lkr9r0fns73cufidg`) deploy `dep-daclbse1egvs7393f820` for `162061c` status `live`; Vercel (`line-up-generator`) production deployment `dpl_J2zWxQjcjqvJqF9j5rsQZRKg4jaQ` `READY` with matching `githubCommitSha`; direct `/health` curl blocked by this session's egress policy (known limitation) — substituted with the platform-status checks above; `frontend/package.json`/`backend/package.json`/`App.jsx`'s `APP_VERSION` all confirmed `3.3.2` on `origin/main`; Supabase (`hzaajccyurlyeweekvma`) security advisors showed no findings beyond pre-existing INFO/WARN items unrelated to this release. Post-promote sync (PR [#1055](https://github.com/kaushikkuberanathan/lineup_generator/pull/1055)) merged same session, `354e707` — `main` and `develop` confirmed content-identical after; `develop`'s soak-lock branch protection was manually unchecked by KK to unblock it and has not yet been re-locked for the next soak cycle. This release includes only the #1049 fix — `App.jsx`'s `enterLegacyScreenForApiRoute()` now runs `resolveDestination()`'s real, identity-scoped membership check for every route carrying a `teamId`, instead of trusting the device's local team cache. Both `API_DRIVEN_HOME`/`API_DRIVEN_ROUTES` remain default off; zero live behavior change for any general user. Full version history lives in `frontend/src/data/versionHistory.js`.

**Note on the `develop`-lock soak mechanism:** this repo enforces the soak freeze via GitHub branch protection's "Lock branch" setting on `develop`, not just a documentation banner. It must be manually unchecked (Settings → Branches → `develop`) before any post-promote sync PR can merge — the "BLOCKED" merge state persists even with every required CI check green until that toggle is flipped. No automation in this repo currently re-locks it for the next soak; that remains a manual step at the start of the next release-prep cycle.

- v3.3.2 (2026-09-03, **promoted to `main`, live in prod**): single-issue security patch. [#1049](https://github.com/kaushikkuberanathan/lineup_generator/issues/1049) (P0, cross-team deep-link route authorization) fixed via PR [#1050](https://github.com/kaushikkuberanathan/lineup_generator/pull/1050), already merged to `develop`: `App.jsx`'s `enterLegacyScreenForApiRoute()` authorized plain team-level routes (roster/schedule/team/lineups) against the device's local, identity-agnostic team cache (`teams.find()`) instead of `resolveDestination()`'s real, identity-scoped Home membership check — which only ran for nested game/lineup routes. Found live during deepened #1033 self-testing (KK's explicit call, taken instead of jumping straight to a real second user): an identity with zero membership on a team reached that team's roster management screen — a real write surface — via a crafted `route=` URL, because the team was still in the device's local cache from a different identity's earlier session on the same browser. Fix: `resolveDestination()` now runs for every route carrying a `teamId`; a present, real Home response's verdict always overrides the local team list, while the pre-existing "no cache yet, fall through to local check" behavior for cold restores/auth-resume is preserved since it wasn't the actual gap. New regression test reproduces the exact live scenario, RED-confirmed before the fix. Both `API_DRIVEN_HOME`/`API_DRIVEN_ROUTES` remain default-off in production — no general user was exposed; any internal tester with the flags manually enabled (KK) was exposed on prod until this promotes. **Patch-version rationale:** `develop` is exactly v3.3.1 (`81745cd7`) plus PR #1048 (a no-op sync merge) plus this fix — nothing else has landed since the last promote. **Verification (2026-09-03, re-run fresh):** frontend 1748/1748 across 168 files (+1 vs. v3.3.1 — the new cross-team route-authorization regression test), backend unit 361/361, lint clean, production build clean, `debt-p0` gate clear (0 open P0). While preparing this release also caught and corrected a stale `FEATURE_MAP.md` row 43 test count (said 18, actual file has 15). **Soak explicitly overridden 2026-09-03 by KK** — the fix had only just merged to `develop` when KK explicitly chose to override the remaining soak window rather than wait, on the grounds that he is the party currently exposed to this exact P0 auth bypass on production (flags manually enabled for #1033 testing) — a stronger justification than this repo's usual "fall season readiness" override rationale. **Promoted to `main` 2026-09-03** (PR [#1054](https://github.com/kaushikkuberanathan/lineup_generator/pull/1054), regular merge, `162061c`) — confirmed a genuine 2-parent merge, merged directly by KK. Prod smoke test same session: Render deploy `dep-daclbse1egvs7393f820` `live`, Vercel deployment `dpl_J2zWxQjcjqvJqF9j5rsQZRKg4jaQ` `READY` on the exact promoted SHA, Supabase advisors clean. Post-promote sync (PR [#1055](https://github.com/kaushikkuberanathan/lineup_generator/pull/1055)) merged same session, `354e707` — `main`/`develop` confirmed content-identical after. Full release-hygiene pass performed same session: `sync-stories-to-issues.js --dry-run` clean, all release PRs/issues confirmed fully labeled, 3 stale orphan branches identified for manual deletion (content already shipped via other merges, unrelated to this release). See `docs/product/ROADMAP.md`'s matching v3.3.2 entry for the full breakdown.
- v3.3.1 (2026-09-03, **promoted to `main`, superseded within hours by v3.3.2**): entirely internal patch — an analytics-accuracy bug fix, deep-link telemetry wiring, and a docs recovery, no new coach-facing capability. [#1041](https://github.com/kaushikkuberanathan/lineup_generator/issues/1041) fixed via PR #1042: `frontend/src/utils/analytics.js` initialized Mixpanel with `ignore_dnt: false`, silently dropping every analytics event (not just `home_*`) for any browser sending Do Not Track, found while reviewing #1033 Stage 3 telemetry — a real prod session generated confirmed-successful backend calls but zero Mixpanel events. Overridden deliberately (no ad network, no data sale, so DNT's advertising opt-out doesn't map onto first-party product analytics); Privacy Policy bumped to version 1.1 disclosing it. A real Ship Gate gap — zero test coverage on the actual `ignore_dnt` value passed to `mixpanel.init()` — found and closed while preparing this release, RED→GREEN mutation-verified. PR #1043 wired `trackHomeDeepLinkResolved`/`trackHomeDeepLinkDenied` (defined since #1012 Phase 1, never called anywhere real) into `App.jsx`'s `enterLegacyScreenForApiRoute()`, closing the observability gap blocking #1033 Stage 4. PR #1044 recovered a real, previously-orphaned 2026-08-28-A session retrospective into `docs/process/SESSION_RETROSPECTIVES.md` — the GitHub issue states it described were independently verified already correct, only the write-up had never merged. **Patch-version rationale:** fixes an existing bug and wires already-designed telemetry into its intended call site; nothing here is new user-facing surface, unlike v3.3.0. **Verification (2026-09-03, re-run fresh):** frontend 1747/1747 across 168 files (+1 vs. v3.3.0 — the new `ignore_dnt` regression test), backend unit 361/361, lint clean, production build clean, `debt-p0` gate clear (0 open P0). **Soak explicitly overridden 2026-09-03 by KK, citing fall season launch readiness** — the 24h `develop` soak clock had only just started (well under an hour old) when KK explicitly chose to override it and proceed straight to the `main` promote, same pattern as v2.9.0 through v3.3.0's overrides. **Promoted to `main` 2026-09-03** (PR [#1047](https://github.com/kaushikkuberanathan/lineup_generator/pull/1047), regular merge, `81745cd7`) — confirmed a genuine 2-parent merge. Prod smoke test same session confirmed clean; post-promote sync (PR [#1048](https://github.com/kaushikkuberanathan/lineup_generator/pull/1048)) merged same session. Superseded within hours by v3.3.2's promote (see above) — v3.3.1 was live in production for only a short window before the #1049 P0 fix promoted on top of it. See `docs/product/ROADMAP.md`'s matching v3.3.1 entry for the full breakdown.
- v3.3.0 (2026-09-02, **promoted to `main`, superseded by v3.3.1/v3.3.2**): comprehensive release-prep pass at KK's request ("line up scope and documents"), bundling everything merged to `develop` since the v3.2.0 promote. Post-promote sync/doc correction through PRs #1007/#1008 (routine). Schedule card redesign and My Team/Roster/Bottom-Nav/PWA/Supabase hardening through PR #1009 (closes #1002): plain-language final score and result replacing compact codes, cards reorganized into game/assignment/score/action regions, duplicated team/season-record info removed. Player deep-link reload navigation fix through PR #1011 (closes #1010): a real bug where a direct or history-restored player-profile URL left stale navigation state. API-Driven Architecture Redesign Phase 0 + Phase 1 (initiative #1012, baseline doc `docs/product/API_DRIVEN_ARCHITECTURE_REDESIGN.md`) through PRs #1034/#1035/#1036 — 19 stories (#1015-#1032) delivering an authenticated Home read-model API, frontend API client + private cache, canonical route parsing/destination resolution, and a redesigned Team Hub, **entirely behind `API_DRIVEN_HOME`/`API_DRIVEN_ROUTES`, both default off, zero live behavior change**; two real gaps (unverified cross-team gameId/lineupId ownership, an unwired pending-destination auth-resume flow) were found and fixed with RED-to-GREEN evidence while closing out #1032. #1033 stays open past this release — docs reconciliation shipped in PR #1036, but the staged flag rollout and legacy Home retirement both require production first. **Minor-version rationale:** the Schedule card redesign is a real, if modest, coach-facing change; the API-driven work, though dark, is a substantial new subsystem large enough to size the bump above patch, matching the v3.0.0/v2.9.0 convention. **Verification (2026-09-02, re-run fresh on a clean `develop` checkout, not carried forward):** frontend 1745/1745 across 168 files, backend unit 361/361, lint clean, production build clean, `debt-p0` gate clear (0 open P0). **Real-device preview validation confirmed complete by KK (2026-09-02).** **Soak explicitly overridden 2026-09-02 by KK** — the 24h `develop` soak clock had only just started (minutes, not hours) when KK explicitly chose to override it and proceed straight to the `main` promote, same pattern as v2.9.0/v2.10.0/v2.11.0/v2.12.0/v2.14.0/v3.0.0's overrides. **Promoted to `main` 2026-09-02** (PR [#1039](https://github.com/kaushikkuberanathan/lineup_generator/pull/1039), regular merge, `dbaa769`) — confirmed a genuine 2-parent merge. Prod smoke test same session: Render deploy `dep-dacabpmk1f9s73ft80hg` `live`, Vercel deployment `dpl_73S6otUovwoUFGiUXVCx7Rw8wrVd` `READY` on the exact promoted SHA, `/health` `200` with `version: "3.3.0"`, `db: "ok"`, Supabase advisors clean. Post-promote sync (PR #1040) in progress same session. See `docs/product/ROADMAP.md`'s matching v3.3.0 RELEASE CANDIDATE entry for the full PR-by-PR breakdown.
- v3.2.0 (2026-09-02, **promoted to `main`, live in prod**): #355 live-scoring RLS lockdown (PR #994, migrations 033+031 already applied to DEV+PROD prior to promote); maintenance-wave-2 cleanup (PR #995); My Team dashboard redesign with player profiles and primary Schedule navigation (PR #1001, #1002); v3.1.1 promotion-status doc correction (PR #1003). Local release validation before promote: frontend 1550/1550 across 146 files, backend unit 299/299 across 59 suites, lint clean, production build passed, `debt-p0` clear. **Promoted to `main` 2026-09-02** (PR #1006, regular merge, `aa519bf`) — confirmed a genuine 2-parent merge. Prod smoke test same session: Render deploy `dep-dabo6ne1egvs739b7mrg` `live`, Vercel deployment `dpl_8Y5bq29mZSVyhtqn7wp2gb2qcTiE` `READY` on the exact promoted SHA, `/health` `200` with `version: "3.2.0"`, `db: "ok"`, Supabase advisors clean. `develop`'s soak-freeze branch lock was manually unchecked by KK to unblock the post-promote sync once prod was confirmed live. Post-promote sync (PR #1007) merged the same session, `ec76dec` — `main` and `develop` confirmed content-identical after.
- v3.1.1 (2026-08-31, **promoted to `main`, live in prod**): migration 032 security hardening and RLS coverage (PR #970); QA coverage for player mapping, logout, auth rejection diagnostics, Songs, PWA install, demo-team loading, Roster, coach-PIN unlock, and Bottom Nav (PRs #971-#976/#980); coach-readable product-activity cleanup (PR #978); Phase 4C status and production-documentation reconciliation (PRs #979/#988). Patch bump: no new coach-facing capability. Bench fairness #942/#952 already shipped in v3.1.0. **Promoted to `main` 2026-08-31** (PR [#991](https://github.com/kaushikkuberanathan/lineup_generator/pull/991), regular merge, `a6a7c5a`) — confirmed a genuine 2-parent merge via direct `git show -s --format='%P'` inspection, not just the GitHub UI; `APP_VERSION`/both `package.json` files confirmed `3.1.1` on `origin/main`. Prod deploy verified per PR #992's own recorded evidence (Vercel deployed the exact merge SHA, Render status `live` on the same SHA, prod frontend 200 + v3.1.1 bundle, `/ping` 200, `/health` 200 with `version: v3.1.1` and `database: ok`) — not independently re-verified in this correction pass since the Render/Vercel MCP connectors are unauthenticated in this session. Post-promote sync (PR #992, `sync: merge v3.1.1 production state back into develop`) merged same-session, confirming `main`/`develop` trees identical immediately after. Tracker #989 remains open (per its own scope, PRs #994/#995 landing on `develop` afterward don't close it out). **Not in this release:** see the "Not included in v3.1.1" paragraph above.
- v3.1.0 (2026-08-30, **promoted to `main`, live in prod**): versioned legal-doc experience + registration consent gate (PRs #907/#910/#913), with service-role-only version audit records through migration 028; Game Day scoreboard/batting-order fixes (#922/#924/#925, PR #923); bench-rotation fairness (#942/#952); pending completed-game retry (#921, PR #933); behavior-preserving SharedView/Support extraction and schedule-hydration cleanup (PRs #934-#937); executable Vitest file-inventory guard plus email/ops contract coverage (PRs #929-#931). Promoted through PR #959 as genuine two-parent merge `02abfc0`; production frontend and backend health were verified, then main was synced back into develop through PR #960.
