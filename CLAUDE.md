...
## Current Version
**v3.3.0** — **promoted to `main`, live in production** as of 2026-09-02 (PR [#1039](https://github.com/kaushikkuberanathan/lineup_generator/pull/1039), merge commit `dbaa769`, "release: promote v3.3.0 to production", confirmed genuine 2-parent merge — parents `aa519bf` (prior `main`) + `9f2cbe3` (`develop` tip)). No dedicated release tracker issue (unlike v3.2.0's #1004) — this was a comprehensive release-prep pass assembled directly from everything merged to `develop` since the v3.2.0 promote, at KK's explicit request ("line up scope and documents"). Scope was the full `develop` tip as of PR #1038 (`9f2cbe3`), landed via PR #1037 (`release: prepare v3.3.0`, `e9d2ac9`) and PR #1038 (soak-override logging, `9537e27`). **Soak explicitly overridden by KK (2026-09-02)** — real-device preview validation confirmed complete, and the 24h `develop` soak clock (only minutes old at the time) was explicitly overridden to proceed straight to this promote, same documented pattern as v2.9.0/v2.10.0/v2.11.0/v2.12.0/v2.14.0/v3.0.0's overrides; the literal `confirmed — push to main` gate phrase was provided before the merge. **Verified directly after promote, not just asserted:** Render backend (`lineup-generator-backend`, `srv-d70lkr9r0fns73cufidg`) deploy `dep-dacabpmk1f9s73ft80hg` for `dbaa769` status `live`; Vercel (`line-up-generator`) production deployment `dpl_73S6otUovwoUFGiUXVCx7Rw8wrVd` `READY` with matching `githubCommitSha`; prod `GET /health` returned `200` with `version: "3.3.0"` and `db: "ok"`; `frontend/package.json`/`backend/package.json`/`App.jsx`'s `APP_VERSION` all confirmed `3.3.0` on `origin/main`; Supabase (`hzaajccyurlyeweekvma`) security advisors showed no findings beyond pre-existing INFO/WARN items unrelated to this release. This release includes the Schedule card redesign and My Team/Roster hardening (PR #1009, closes #1002), the player deep-link reload navigation fix (PR #1011, closes #1010), and the API-Driven Architecture Redesign Phase 0 + Phase 1 (initiative #1012, 19 stories, PRs #1034/#1035/#1036) — entirely behind `API_DRIVEN_HOME`/`API_DRIVEN_ROUTES`, both default off, zero live behavior change in this release. Full version history lives in `frontend/src/data/versionHistory.js`.
