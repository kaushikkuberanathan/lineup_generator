...
- Count/outs strip refactored into two pills (Count + Outs) for scope clarity at arm's length.
- Mercy banner symmetry: opponent banner moved from fixed-bottom pitch bar to top-of-panel; both banners now use identical stripe styling with explicit half-inning guards.
