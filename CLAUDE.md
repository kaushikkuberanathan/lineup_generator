...
## Current Version
**v3.2.0** — **promoted to `main`, live in production** as of 2026-09-02 (PR [#1006](https://github.com/kaushikkuberanathan/lineup_generator/pull/1006), merge commit `aa519bf`, "release: promote v3.2.0 to production", confirmed genuine 2-parent merge — parents `a6a7c5a` (prior `main`) + `9d30339` (`develop` tip)). Release tracker: #1004. Scope was frozen at `develop` commit `e79487d` against production v3.1.1 merge `a6a7c5a`, landed on `develop` via PR #1005 (`chore(release): prepare v3.2.0 candidate #1004`, `14c4f04`). Ship Gate authorization, the 24h soak override, and real-device preview validation were all given explicitly by KK in-session on 2026-09-02; the literal `confirmed — push to main` gate phrase was provided before the merge. **Verified directly after promote, not just asserted:** Render backend (`lineup-generator-backend`) deploy `dep-dabo6ne1egvs739b7mrg` for `aa519bf` status `live`; Vercel (`line-up-generator`) production deployment `dpl_8Y5bq29mZSVyhtqn7wp2gb2qcTiE` `READY` with matching `githubCommitSha`; prod `GET /health` returned `200` with `version: "3.2.0"` and `db: "ok"`; `frontend/package.json`/`backend/package.json`/`App.jsx`'s `APP_VERSION` all confirmed `3.2.0` on `origin/main`; Supabase (`hzaajccyurlyeweekvma`) security advisors showed no findings beyond pre-existing INFO/WARN items unrelated to this release. Post-promote sync (PR [#1007](https://github.com/kaushikkuberanathan/lineup_generator/pull/1007), merge commit `ec76dec`) confirmed `main`/`develop` content-identical immediately after. This release includes the #355 live-scoring RLS lockdown (PR #994, migrations 033+031 already applied and verified on DEV+PROD prior to this promote — see Known Open Bugs row 4 below), maintenance-wave-2 cleanup (PR #995), the My Team dashboard/player-profiles/primary-Schedule-navigation redesign (PR #1001), and the v3.1.1 promotion-status doc correction (PR #1003). Full version history lives in `frontend/src/data/versionHistory.js`.
