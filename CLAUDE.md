...
## Test Suite
Changes to `lineupEngineV2.js`, `scoringEngine.js`, or `playerMapper.js` → must pass frontend `npm test` (Vitest, 1401 passing / 1 skipped as of v3.0.0, 2026-08-29 — fewer may be observed locally on Windows due to Bug #7 cold-start flake). Backend unit suite is 269/269 (see `backend/CLAUDE.md` § Test Suite for that count's own history). This line had sat at the stale v2.12.0-era 1090 figure through multiple subsequent releases — flagged as a carry-forward in the 2026-08-24-B and 2026-08-26-A session retros before finally being corrected in v2.15.0-prep (1368/1, 2026-08-26); corrected again for PR #867's `FAQSection.test.jsx` growth (6→15), and again here for v3.0.0 (1377→1401 frontend; backend unit 254→269, see `backend/CLAUDE.md` for the per-file breakdown).
