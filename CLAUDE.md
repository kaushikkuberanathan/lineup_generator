...
## Test Suite
Changes to `lineupEngineV2.js`, `scoringEngine.js`, or `playerMapper.js` → must pass frontend `npm test` (Vitest, 1377 passing / 1 skipped as of the Story 333/#865 Help redesign, 2026-08-28 — fewer may be observed locally on Windows due to Bug #7 cold-start flake). Backend unit suite is 254/254 (unchanged by this PR; see `backend/CLAUDE.md` § Test Suite for that count's own history). This line had sat at the stale v2.12.0-era 1090 figure through multiple subsequent releases — flagged as a carry-forward in the 2026-08-24-B and 2026-08-26-A session retros before finally being corrected in v2.15.0-prep (1368/1, 2026-08-26); corrected again here after PR #867 grew `FAQSection.test.jsx` from 6 to 15 tests as part of the Help/FAQ redesign.
...
## Current Version
**v2.15.0** — August 2026, live in prod. **Story 333/#865 (Help redesign, documented as v2.15.1 in `VERSION_HISTORY` — see the entry immediately below) has merged to `develop` (PR #867, 2026-08-28) but not yet promoted, and the version number itself has not been bumped yet (APP_VERSION and both `package.json` files are still `2.15.0`) — that 2.15.1 label is provisional documentation only, pending an explicit release-cut decision.** Full version history in `VERSION_HISTORY` constant in `frontend/src/data/versionHistory.js`.
