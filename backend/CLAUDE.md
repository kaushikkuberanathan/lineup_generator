...
Unit suite total: **199** (verified via `npm run test:unit`, 2026-08-25, admin.html bypass remediation closeout — 0 fail / 0 skipped, per-file counts below individually re-verified via `node --test <file>` the same session, not estimated or carried forward from the prior 147 figure). Story 99 closed 2026-07-31; see ROADMAP.md Story 99 for the closure writeup.
