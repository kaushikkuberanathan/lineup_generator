...
Unit suite total: **249** (verified via `npm run test:unit`, 2026-08-26, #474 closure — up from 232, +17: `adminRequests.test.js` (3), `adminMembers.test.js` (4), `adminMembershipActions.test.js` (10, covering update-role/reset-access/suspend together) — the last 3 of #474's originally-6-route list to gain authorized-path coverage; `GET /admin/deny-link` turned out to already have it via `adminLinkToken.route.test.js`'s LT-7, narrowing 6 to 5 before this pass, confirmed by re-reading that file directly rather than trusting #474's own stale count). Story 99 closed 2026-07-31; see ROADMAP.md Story 99 for the closure writeup.
