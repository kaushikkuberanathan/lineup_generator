...
Unit suite total: **227** (verified via `npm run test:unit`, 2026-08-26, #406/#410 test-health survey follow-up — up from 220, +7: `adminFeedback.test.js` (6, new file) + 1 new 401-rejection case in `admin.auth.test.js`, closing the `GET /api/v1/feedback` gap the survey found — that route previously had zero coverage of any kind). Story 99 closed 2026-07-31; see ROADMAP.md Story 99 for the closure writeup.
